...
const NAV_ITEMS = [
  { href: '/dashboard.html',        label: 'Dashboard',         icon: 'dashboard',     mobile: 'Home',     mobileIcon: 'home' },
  { href: '/study_notes.html',      label: 'Study Notes',       icon: 'menu_book',     mobile: 'Notes',    mobileIcon: 'menu_book' },
  { href: '/practice_session.html', label: 'Practice Sessions', icon: 'edit_square',   mobile: 'Practice', mobileIcon: 'edit_square' },
  { href: '/growth_history.html',   label: 'History',           icon: 'history',       mobile: 'History',  mobileIcon: 'history' }
];

const FOOTER_ITEMS = [
  { href: '/account_settings.html', label: 'Settings', icon: 'settings' },
...
export function renderShell({ active } = {}) {
  ...
  sideNav.innerHTML = `
    ...
    <div class="px-3 py-4 border-t border-outline-variant space-y-1">
      ...
      <a href="/practice_session.html" class="mt-3 w-full h-12 bg-primary text-on-primary rounded-lg font-bold text-label-md flex items-center justify-center gap-2 hover:opacity-90">
  ...
  topBar.innerHTML = `
    ...
    <div class="flex items-center gap-2">
      ...
      <a href="/account_settings.html" class="w-9 h-9 rounded-full bg-primary text-on-primary flex items-center justify-center text-label-md font-bold">${initials.toUpperCase()}</a>
    </div>
  `;
